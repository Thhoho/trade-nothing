...
3. Harvesting unresolved attacks into Local Issues; reminders/webhooks are explicit opt-ins.
...
try:
    ...
    DEFAULT_EVOLUTION_PATH = os.path.join(SKILL_DIR, "Methodology_Evolution.md")
...
def harvest_unresolved_attacks(topic: str, state_file: str, raw_attacks: str,
                               notify: bool = False, send_webhook: bool = False):
    """Harvest unresolved attacks; external notifications are explicit opt-ins."""
    attacks_list = []
    posterior = 50.0
    topic_slug = generate_topic_slug(topic)
    
    if not state_file:
        state_dir = os.path.join(BASE_SCRATCH_DIR, "state")
...
"""
        ...
        if notify and os.path.exists(REMINDERS_SCRIPT):
...
def main():
    ...
    parser.add_argument("--notify", action="store_true", help="Opt in to OS reminder creation during --harvest")
    parser.add_argument("--webhook", action="store_true", help="Opt in to webhook delivery during --harvest")
    ...
    if args.extract:
        ...
        harvest_unresolved_attacks(args.topic, args.state_file, args.unrefuted_attacks,
                                   notify=args.notify, send_webhook=args.webhook)
